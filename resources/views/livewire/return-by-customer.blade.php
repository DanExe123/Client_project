  ...
  <div x-cloak x-data="returnToSupplierUI" class="space-y-2">
        ...
        <tbody class="divide-y divide-gray-100 border-t border-gray-100">
          <template x-for="(rs, index) in returnList" :key="index">
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4"
                x-text="formatPrice(rs.products.reduce((total, p) => total + (p.qty * p.unitPrice), 0))"></td>
            </tr>
          </template>
    ...
    <div x-show="showForm" x-transition class="bg-white border border-gray-200 rounded-lg p-6 space-y-4 mt-5">
      ...
      <div class="overflow-auto mt-4">
        <table class="w-full text-sm border">
          ...
          <tbody>
            <template x-for="(item, index) in form.products" :key="index">
              <tr>
                <td class="p-2"><input type="text" x-model="item.barcode" class="w-full border px-2 py-1 rounded" />
                </td>
                <td class="p-2">
                ...
                <td class="p-2"><input type="number" x-model="item.unitPrice" class="w-full border px-2 py-1 rounded" />
                </td>
        ...
        <x-button green label="Add Product" class="mt-2 ml-2 mb-1"
          @click="form.products.push({ barcode: '', product: '', qty: 1, unitPrice: 0 })" />
