                    ...
                    <div class="overflow-auto rounded-lg border border-gray-200 shadow-md">
                        <table class="min-w-[800px] w-full border-collapse bg-white text-left text-sm text-gray-500">
                            ...
                            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                                ...
                                @else
                                    @if ($tab === 'Approved')
                                        @foreach ($orders as $receiving)
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-4 py-4">
                                                    <input type="checkbox" wire:click="selectedPo({{ $receiving->id }})"
                                                        @if (in_array($receiving->id, $selectedpoId)) checked @endif />
                                                </td>
                                                <td class="px-6 py-4">{{ $receiving->po_number }}</td>
                                                <td class="px-6 py-4">{{ $receiving->supplier->name ?? 'N/A' }}</td>
                                                <td class="px-6 py-4">{{ \Carbon\Carbon::parse($receiving->order_date)->format('Y-m-d') }}</td>
                                                <td class="px-6 py-4">{{ $receiving->receipt_type }}</td>
                                                <td class="px-6 py-4 text-green-600 font-semibold">Approved</td>
                                                <td class="px-6 py-4">₱{{ number_format($receiving->grand_total, 2) }}</td>
                                                <td class="px-6 py-4">
                                                    <a href="{{ route('view-receiving-details', ['id' => $receiving->id]) }}">
                                                    ...
                                                    <div class="flex gap-2">
                                                        ...
                                                        @if ($tab === 'For Approval')
                                                            <a href="{{ route('recieving.cancel', ['id' => $po->id]) }}"
                                                                ...
                                                                <x-button outline negative label="Cancel" />
                                                            </a>
                                                        @endif
                                                    </div>
                                                </td>
