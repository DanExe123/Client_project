...
<div id="toast-default" 
    class="fixed top-4 right-4 flex items-center w-full max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow-lg z-50 transition-transform transform duration-300 ease-in-out"
    role="alert"
    x-data="{ show: true }"
    x-init="setTimeout(() => show = false, 5000)" 
    x-show="show"
    x-transition:enter="transition ease-out duration-500 transform"
    x-transition:enter-from="opacity-0 translate-y-4"
    x-transition:enter-to="opacity-100 translate-y-0"
    x-transition:leave="transition ease-in duration-300 transform"
    x-transition:leave-from="opacity-100 translate-y-0"
    x-transition:leave-to="opacity-0 translate-y-4"
>
    <div class="inline-flex items-center justify-center shrink-0 w-8 h-8 text-blue-500 bg-blue-100 rounded-lg dark:bg-blue-800 dark:text-blue-200">
        ...
        <span class="sr-only">Sent Email</span>
    </div>
